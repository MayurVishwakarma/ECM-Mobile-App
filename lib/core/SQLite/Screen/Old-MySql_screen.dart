  ...
  void initState() {
    ...
    try {
      if (widget.Source == 'oms') {
        fatchFirstloadOms(widget.deviceId!);
...
//oms
  Future fatchFirstloadOmsoff(int omsid, String processName) async {
    if (omsid != 0) {
      ...
      Listdata = await ListViewModel.instance.fetchByOmsId(omsid);
      listdistinctProcesss = await ListModel.instance.fetchAll();
      datas = await DBSQL.instance.fetchByType(omsid, processIds!, processName);
      setState(() => isLoading = false);
      subProcessName = Set();
  ...
  Future fatchFirstloadAmsoff(int amsid, String processName) async {
    if (amsid != 0) {
      ...
      Listdata = await ListViewModel.instance.fetchByAmsId(amsid);
      listdistinctProcesss = await ListModel.instance.fetchAll();
      datas = await DBSQL.instance.fetchByType(amsid, processIds!, processName);
      setState(() => isLoading = false);
      subProcessName = Set();
      for (var element in datas) {
        setState(() {
  ...
  Future fatchFirstloadLoraoff(int getwayid, String processName) async {
    if (getwayid != 0) {
      ...
      listdistinctProcesss = await ListModel.instance.fetchAll();
      datas =
          await DBSQL.instance.fetchByType(getwayid, processIds!, processName);
      // setState(() => isLoading = false);
      subProcessName = Set();
      for (var element in datas) {
        setState(() {
  ...
  Future fatchFirstloadRmsoff(int rmsid, String processName) async {
    if (rmsid != 0) {
      ...
      Listdata = await ListViewModel.instance.fetchByRmsId(rmsid);
      listdistinctProcesss = await ListModel.instance.fetchAll();
      datas = await DBSQL.instance.fetchByType(rmsid, processIds!, processName);
      // setState(() => isLoading = false);
      subProcessName = Set();
      for (var element in datas) {
        setState(() {
  ...
  Future fatchdataSQL(String selectprocess, int processId) async {
    setState(() => isLoading = true);
    allchecklist = await DBSQL.instance.fetchByDevice(widget.deviceId!);
    Listdata = await ListViewModel.instance.fetchAll();
    listdistinctProcesss = await ListModel.instance.fetchAll();
    datas = await DBSQL.instance.fetchByType(
      widget.deviceId!,
      processId,
      widget.Source!,
    );
...
//update fun
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                for (int j = 0; j <= datas.length; j++) {
                  ...
                  datas[j].deviceId = widget.deviceId;
  ...
  Future<void> getECMData(String processName) async {
    final selectedProcessId = listdistinctProcesss
        .singleWhere((item) => item.processName == processName)
        .processId;

    setState(() {
      processId = selectedProcessId;
      processIds = selectedProcessId;
    });

    // Call your loaders (no need to wrap these in setState)
    await Future.wait([
      fatchFirstloadAmsoff(widget.deviceId!, processName),
      fatchFirstloadOmsoff(widget.deviceId!, processName),
      fatchFirstloadLoraoff(widget.deviceId!, processName),
      fatchFirstloadRmsoff(widget.deviceId!, processName),
    ]);
  }
/*getECMData(String processName) async {
    ...
    try {
      if (_checkList != null) {
        ...
        if (flagCounter == subProcessName!.length) {
          ...
          await DBSQL.instance.deleteByDeviceProcess(
  ...
  DeviceAllDAta() async {
    // if (widget.Source == 'oms') {
    nodedata = await DBSQL.instance.fetchByDevice(widget.deviceId!);
    // }
  ...
  Future listdatacheckup() async {
    ...
    await ListViewModel.instance.deleteOms(widget.modelDatas!.omsId!);
    if (widget.modelDatas!.amsId != 0) {
      await ListViewModel.instance.deleteAms(widget.modelDatas!.amsId!);
    }
    if (widget.modelDatas!.rmsId != 0) {
      await ListViewModel.instance.deleteRms(widget.modelDatas!.rmsId!);
    }
    if (widget.modelDatas!.gateWayId != 0) {
      await ListViewModel.instance.deleteGateway(widget.modelDatas!.gateWayId!);
    }
  }
        ...
        if (json["Status"] == "Ok") {
          if (subprocessId == subProcessName!.length) {
            ...
            await DBSQL.instance
                .deleteByDeviceProcess(imageList.first.deviceId!, processId!);
            Navigator.pop(context);
            Navigator.pop(context);
          }
          return true;
...
//display function
  ...
  Future<List<ECM_Checklist_Model>> getECMCheckListByProcessId(
    ...
    try {
      List<ECM_Checklist_Model> result = [];
      result = await DBSQL.instance.fetchByType(_deviceId, _processId, _source);
      return result;
    } on Exception catch (_, ex) {
      throw Exception("API Consumed Failed");
    }
  ...
  Future<List<ECM_Checklist_Model>> getOMSECMCheckListByProcessId(
    ...
    try {
      List<ECM_Checklist_Model> result = [];
      result = await DBSQL.instance.fetchByProcess(_deviceId, _processId);
      return result;
    } on Exception catch (_, ex) {
      throw Exception("API Consumed Failed");
    }
