...
class _NodeDetails_SQLState extends State<NodeDetails_SQL> {
  ...
  Future<void> _initConnectivity() async {
    InternetStatus status =
        (await InternetConnection().onStatusChange) as InternetStatus;
...
//fatch for rms
  ...
  Future getImage(ImageSource media, int index) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Image size is greater than 1.5 MB'),
  ...
  Widget _buildChecklistItem(dynamic item, int index) {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
    ...
    try {
      if (
        ...
        String? testpath = await uploadImageAndGetPath(
            image.path, widget.Source!.toUpperCase(), deviceId);
        return testpath;
      } else {
        return '';
      }
    ...
    try {
      ...
      await Future.wait(imageList
        ...
        String? imagePathValue = await uploadImageAndGetPath(
            element.image!.path, widget.Source!.toUpperCase(), deviceId);
...
// send to server
  ...
  Future<bool> insertCheckListDataWithSiteTeamEngineer_func(
    ...
    try {
      ...
      projectName = project!.replaceAll(' ', '_');
      ...
      await Future.wait(imageList
          ...
          .map((element) async {
        String? imagePathValue = await uploadImageAndGetPath(
            element.image!.path, widget.Source!.toUpperCase(), omsId!);
        if (imagePathValue!.isNotEmpty) {
          element.value = imagePathValue;
          uploadflag++;
        }
