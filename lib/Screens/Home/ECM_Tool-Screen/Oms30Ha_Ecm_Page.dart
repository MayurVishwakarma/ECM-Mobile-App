// ignore_for_file: prefer_const_constructors, file_names, prefer_const_literals_to_create_immutables, sort_child_properties_last, import_of_legacy_library_into_null_safe, use_key_in_widget_constructors, library_private_types_in_public_api, unused_import, unused_element, prefer_interpolation_to_compose_strings, avoid_print, prefer_is_empty, non_constant_identifier_names, curly_braces_in_flow_control_structures, prefer_final_fields, unnecessary_new, use_build_context_synchronously, no_leading_underscores_for_local_identifiers, unused_local_variable, unused_catch_stack, must_be_immutable, prefer_typing_uninitialized_variables, prefer_collection_literals, unrelated_type_equality_checks, avoid_unnecessary_containers
...
import 'package:ecm_application/Screens/Home/ECM_Tool-Screen/NodeDetails_30Ha.dart';
...
class Oms30haPage extends StatefulWidget {
  String? ProjectName;
  String? Source;
  Oms30haPage({
    required,
  ...
  @override
  State<Oms30haPage> createState() => _Oms30haPageState();
}

class _Oms30haPageState extends State<Oms30haPage> {
  ...
  getDropDownAsync() async {
    ...
    await getProcessid(source: 'below30ha').then((values) {
      ProcessList = [];
      ProcessStatusList = [];
      var processList = Set();
      for (var e in values) {
            ...
            return InkWell(
              onTap: () async {
                ...
                Navigator.pushAndRemoveUntil(
                  context,
                  MaterialPageRoute(
                      builder: (context) => NodeDetails30Ha(
                          _DisplayList![index],
                          projectName,
                          source,
                          listdatas)),
  ...
  getprocessstatus(String pro, int proStatus) {
    ...
    try {
      if (pro.toLowerCase().contains('DryCommissioning')) {
        if (proStatus == 1) {
  ...
  getProStatus(String proStatus, PMSListViewModel model) {
    ...
    try {
      ...
      if (proStatus.contains('trench'))
        status = int.tryParse(model.trenching!);
      else if (proStatus.contains('pipe inst'))
        status = int.tryParse(model.pipeInatallation!);
    } catch (_) {}
    return status;
  }
  ...
  void _firstLoad() async {
    ...
    try {
      ...
      final res = await http.get(Uri.parse(
          'http://wmsservices.seprojects.in/api/PMS/ECMReportStatus?Search=$_search&areaId=$area&DistributoryId=$distibutory&Process=$process&ProcessStatus=$processStatus&pageIndex=$_page&pageSize=$_limit&Source=below30ha&conString=$conString'));
