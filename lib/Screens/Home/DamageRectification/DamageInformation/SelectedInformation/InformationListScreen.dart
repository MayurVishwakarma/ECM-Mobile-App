...
import 'package:ecm_application/Screens/Home/DamageRectification/DamageInformation/InfoDetails_CommonScreen.dart';
...
class SelectedInformationList extends StatefulWidget {
  String? ProjectName;
  String? Source;

  SelectedInformationList(
  ...
  @override
  State<SelectedInformationList> createState() =>
      _SelectedInformationListState();
}

class _SelectedInformationListState extends State<SelectedInformationList> {
  @override
  void initState() {
    super.initState();
  ...
  getDamageHistoryData() {
    ...
    try {
      if (widget.Source == 'OMS') {
        getDamageInformationCommon(
                modelData!.omsId!, (widget.Source ?? 'OMS'), 'information')
            .then((value) {
          setState(() {
        ...
        getDamageInformationCommon(
                modelData!.amsId!, widget.Source!, 'information')
            .then((value) {
          setState(() {
            remarkval = value.first.remark ?? "";
            workedondate = (value.first.reportedOn ?? '').toString();
        ...
        getDamageInformationCommon(
                modelData!.rmsId!, widget.Source!, 'information')
            .then((value) {
          setState(() {
            remarkval = value.first.remark ?? "";
            workedondate = (value.first.reportedOn ?? '').toString();
      ...
      } else if (widget.Source?.toUpperCase() == 'LORA') {
        getDamageInformationCommon(
                modelData!.gatewayId!, widget.Source!, 'information')
            .then((value) {
          setState(() {
            remarkval = value.first.remark ?? "";
            workedondate = (value.first.reportedOn ?? '').toString();
