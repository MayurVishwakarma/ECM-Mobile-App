...
import 'package:ecm_application/Screens/Home/DamageRectification/DamageIssues/SelectedIssues/IssueListScreen.dart';
...
class SelectedOmsDamageIssuesScreen extends StatefulWidget {
  SelectedOmsDamageIssuesScreen(
      {Key? key,
      AreaModel? area_,
  ...
  @override
  State<SelectedOmsDamageIssuesScreen> createState() =>
      _SelectedOmsDamageIssuesScreenState();
}

class _SelectedOmsDamageIssuesScreenState
    extends State<SelectedOmsDamageIssuesScreen> {
...
/*
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('List By Selected Issue Counter'),
      ),
            ...
            child: InkWell(
                onTap: () async {
                  ...
                  Navigator.pushAndRemoveUntil(
                    context,
                    MaterialPageRoute(
                        builder: (context) => SelectedIssuesList(
                              _DisplayList![index],
                              projectName,
                              Source!,
                            )),
  ...
  void _firstLoad() async {
    ...
    try {
      ...
      final res = await http.get(Uri.parse(
          'http://wmsservices.seprojects.in/api/infoReport/GetInformationReportStatus_DateFilter?AreaId=$area&DistributoryId=$distibutory&Source=$Source&isFilter=$ListStatus&InfoTypeName=issue&conString=$conString'));
      print(
          'http://wmsservices.seprojects.in/api/infoReport/GetInformationReportStatus_DateFilter?AreaId=$area&DistributoryId=$distibutory&Source=$Source&isFilter=$ListStatus&InfoTypeName=issue&conString=$conString');
